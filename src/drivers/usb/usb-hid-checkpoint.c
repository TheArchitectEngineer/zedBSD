...
static unsigned checkpoint_bus_number(const struct usb_hid_checkpoint *checkpoint);
static void checkpoint_report_submit(struct usb_hid_checkpoint *checkpoint, int error);
static void checkpoint_report_completion(struct usb_hid_checkpoint *checkpoint, int drain_error);
static void checkpoint_completion(struct drv_usb_urb *urb, void *argument);
static int checkpoint_begin_submit(struct usb_hid_checkpoint *checkpoint);
static void checkpoint_end_submit(struct usb_hid_checkpoint *checkpoint);
static int checkpoint_arm(struct usb_hid_checkpoint *checkpoint);
static unsigned checkpoint_take_work(struct usb_hid_checkpoint *checkpoint, int *stopping);
static void checkpoint_worker(void *argument);
static int checkpoint_buffer_size(struct drv_usb_interface *interface, struct drv_usb_endpoint *endpoint, size_t *result);
static int checkpoint_attach(struct drv_usb_interface *interface, const struct drv_usb_id *id);
static void checkpoint_close_admission(struct usb_hid_checkpoint *checkpoint);
static int checkpoint_join_worker(struct usb_hid_checkpoint *checkpoint);
static void checkpoint_report_detach(struct usb_hid_checkpoint *checkpoint, int cancel_error, int drain_error, int join_error);
...
static unsigned
checkpoint_bus_number(
	const struct usb_hid_checkpoint *checkpoint)
{
	unsigned function_result;

	/* Obtains the drv usb bus number result. */
	function_result =
		drv_usb_bus_number(drv_usb_device_bus(checkpoint->device));

	/* Returns the computed result. */
	return function_result;
}

/* Supports the checkpoint report submit operation. */
static void
checkpoint_report_submit(
...
/* Supports the checkpoint report completion operation. */
static void
checkpoint_report_completion(
	struct usb_hid_checkpoint *checkpoint,
	int drain_error)
{
	...
	kern_logf("usb-hid-checkpoint: completion generation=%u usb%u "
		   ...
		   drv_usb_device_address(checkpoint->device),
		   drv_usb_interface_number(checkpoint->interface),
		   checkpoint->completion_count, (unsigned)status,
		   (unsigned)actual, drain_error);
}

/* Supports the checkpoint completion operation. */
static void
...
/* Supports the checkpoint begin submit operation. */
static int
checkpoint_begin_submit(
	struct usb_hid_checkpoint *checkpoint)
{
...
/* Supports the checkpoint end submit operation. */
static void
checkpoint_end_submit(
	struct usb_hid_checkpoint *checkpoint)
{
	unsigned long irq = spin_lock_irqsave(&checkpoint->lock);
...
/* Supports the checkpoint arm operation. */
static int
checkpoint_arm(
	struct usb_hid_checkpoint *checkpoint)
{
...
/* Supports the checkpoint worker operation. */
static void
checkpoint_worker(
	void *argument)
{
...
/* Supports the checkpoint buffer size operation. */
static int
...
/* Supports the checkpoint attach operation. */
static int
checkpoint_attach(
	struct drv_usb_interface *interface,
...
/* Supports the checkpoint close admission operation. */
static void
checkpoint_close_admission(
	struct usb_hid_checkpoint *checkpoint)
{
	struct thread *worker;
	unsigned active;
...
/* Supports the checkpoint join worker operation. */
static int
checkpoint_join_worker(
	struct usb_hid_checkpoint *checkpoint)
{
	struct thread *worker;
	unsigned long irq;
...
/* Supports the checkpoint report detach operation. */
static void
checkpoint_report_detach(
	struct usb_hid_checkpoint *checkpoint,
	int cancel_error,
	int drain_error,
	int join_error)
{
	...
	kern_logf("usb-hid-checkpoint: detach generation=%u usb%u device=%u "
		   ...
		   drv_usb_device_address(checkpoint->device),
		   drv_usb_interface_number(checkpoint->interface),
		   checkpoint->submit_count, checkpoint->completion_count,
		   cancel_error, drain_error, join_error);
}

/* Supports the checkpoint detach operation. */
static int
checkpoint_detach(
	struct drv_usb_interface *interface,
	unsigned flags)
