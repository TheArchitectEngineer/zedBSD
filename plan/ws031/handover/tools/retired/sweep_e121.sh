# WS031 E-121 (N0 record per the E-120 review; DPLL pipe credit from the encoder clock select; DPLL sanitize keeps a PLL with an active pipe): regression of all accepted modes.
...
# usage (on agent-1): bash /tmp/sweep_e121.sh > /tmp/sweep_e121.log 2>&1
set -u
cd ~/zedBSD
TAG=e121
build() {
...
echo "SWEEP COMPLETE: 13/13 (e121)"
