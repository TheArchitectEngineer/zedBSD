# WS031 E-122 (VBT_ONLY; OpRegion service on a SHADOW mailbox: notifier chain, reference lifecycle, ASLE worker; LCD-O synthetic ASLE brightness on the real LCD): regression of all accepted modes + LCD-O.
...
# usage (on agent-1): bash /tmp/sweep_e122.sh > /tmp/sweep_e122.log 2>&1
set -u
cd ~/zedBSD
TAG=e122
build() {
...
echo "SWEEP COMPLETE: 14/14 (e122)"
