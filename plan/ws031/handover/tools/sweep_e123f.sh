# WS031 E-123 (HDMI hotplug receive path always on, IRQ-safe work queue, GSE wiring, OpRegion FIRMWARE backend; VBT_ONLY; OpRegion service on a SHADOW mailbox: notifier chain, reference lifecycle, ASLE worker; LCD-O synthetic ASLE brightness on the real LCD): regression of all accepted modes + LCD-O.
...
# usage (on agent-1): bash /tmp/sweep_e123.sh > /tmp/sweep_e123.log 2>&1
set -u
cd ~/zedBSD
TAG=e123f
build() {
...
build lcdo -DPARITY_LCDO_TEST=1 || exit 1
...
RS=./run-parity-ref-240.sh run lcdo "LCD-O (step|restore|verdict)" "LCD-O verdict: PASS" &&
echo "SWEEP COMPLETE: 14/14 (e123)"
build hdmib -DPARITY_HDMI_B_TEST=1 || exit 1
build dual -DPARITY_DUAL_TEST=1 || exit 1
build dualsh -DPARITY_DUAL_SHARE_TEST=1 || exit 1
RS=./run-parity-ref-240k.sh run hdmib "HDMI-B (verdict|input|preflight)" "HDMI-B verdict: PASS" &&
RS=./run-parity-ref-240k.sh run dual "DUAL (verdict|frames|window)" "DUAL verdict: PASS" &&
RS=./run-parity-ref-240k.sh run dualsh "DUAL-SHARED (verdict|frames|window)" "DUAL-SHARED verdict: PASS" &&
echo "SWEEP COMPLETE: 17/17 (e123f)"
