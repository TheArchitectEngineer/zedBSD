...
| 基盤 | `mmio.c`（register access、forcewake、`intel/forcewake-ranges.inc`）、`pci.c`、`dma.c`、`runtime-pm.c`、`firmware.c`（`/lib/firmware/<name>` を VFS から読む。root の mount を最長 30 秒待つ）、`sync.c`（時間・待ち・completion）、`workqueue.c`（work／delayed work）、`trace.c` | uncore、DMA API、completion／waitq、runtime PM。旧 `parity/osdep/*`、`backend_*.c`、`wait.c` |
...
| engine・実行 | `engine.c`、`context.c`（context image、`intel/lrc-offsets.h`）、`request.c`、`submit.c`（execlists、CSB）、`defaults.c`（`__engines_record_defaults`）、`migrate.c`、`pxp.c`、`irq.c`（GT 側と top-level handler） | `intel_gt_init` 一式（execlists）。旧 `gt_engine.c`、`gt_lrc.c`、`gt_request.c`、`gt_submit.c`、`gt_resume.c`、`gt_defaults.c`、`gt_migrate.c`、`pxp.c`、`irq.c` の GT 半分 |
...
| 表示 | `display/`: `display.c`（段階関数と入口）、`power.c`、`clock.c`（CDCLK、DPLL）、`phy.c`、`dmc.c`、`modeset.c`・`state.c`、`pipe.c`、`plane.c`、`scanout.c`、`present.c`、`vblank.c`、`interrupts.c`、`ddi.c`、`dp.c`・`dp-sink.c`、`aux.c`、`panel.c`・`panel-backlight.c`、`edid.c`・`edid-read.c`、`hdmi.c`・`hdmi-mode.c`、`gmbus.c`、`vbt.c`、`opregion.c`、`takeover.c`、`hotplug.c`、`watermark.c`、`color.c`、`diagnostics.c`。環境ヘッダ `*-internal.h`、`vbt.h`。定義は `intel/` の表示の header（DisplayPort は `intel/dp.h`） | `intel_display_*`、`intel_ddi.c`、`intel_dp*.c`、`intel_pps.c`、`intel_bios.c`、`intel_opregion.c`、`skl_*.c` ほか。旧 `parity/` の表示系（`power_domains.c`、`cdclk.c`、`display_*.c`、`dmc.c`）と `parity/{lcd,dp,vbt}/` |
| compiler | `compiler/spirv.c`（SPIR-V → scalar IR）、`compile.c`（IR → Gen12 EU）、`eu.c`（命令 encoder、`intel/eu-encoding-gen12.h`） | 独立実装。旧 `vk/spirv.c`、`compile.c`、`eu.c` |
| render（Vulkan executor） | `render/`: `vulkan.c`・`dispatch.c`（opcode の振分け）、`transport.c`・`codec.c`（`render/vulkan-codec.inc`、`handover/tools/gen_vk_server_codec.py` が生成）、`instance.c`、`objects.c`・`object.c`・`memory.c`・`image.c`・`descriptor.c`、`pipeline.c`・`pipeline-prepare.c`・`render-pass.c`、`command.c`（command buffer と記録）、`state.c`・`batch.c`・`draw.c`・`blit.c`・`math.c`、`fence.c`・`sync.c`、`reply.c` | 独立実装。旧 `vk/`（libvulkan から到達する部分だけ。到達しない `res`／`pipe`／`cmdbuf`／`wsi`／`display` は廃止、計画 §5.1） |
| intel | `intel/`: Linux／Mesa 由来の定義と表（系統別の header、使う .c が個別に include）。GT は `gt-regs.h`（GT・engine・interrupt・context・GTT の register）、`commands.h`（MI・blitter・PIPE_CONTROL）、`lrc-offsets.h`（Gen12 context image の offset 表）、`pci-ids.h`、`gt-power.h`（RC6／RPS）、`workarounds.h`、`mocs.h`（MOCS と PAT）、`bits.h`（bit helper、zedBSD の定義）、Mesa の 3D state は `genxml.h`、EU 命令の encoding は `eu-encoding-gen12.h`（Mesa）、表示の系統別 header は `intel/` 直下（`dp.h`、`ref.h`、`ref-inlines.h`、`connector.h`、`power.h`、`power-set.h`、`plane.h`、`fourcc.h`、`psr.h`、`mreg.h`、`ddi.h`、`trans.h`、`phy.h`、`wm.h`、`fixed.h`、`wm-dbuf.h`、`hotplug.h`、`hotplug-types.h`、`opregion.h`、`vbt.h`、`vbt-defs.h`、`clock.h`）、配列の本体 `engine-table.inc`・`forcewake-ranges.inc`（`handover/tools/gen_fw_ranges.py` が生成）・`mocs-table.inc`・`mcr-ranges.inc`。firmware の byte 配列は持たない（2026-09-22: DMC は package `i915-firmware` の `/lib/firmware/i915/` から `firmware.c` が読む。対象機 VBT は試験 build `I915_TEST_VBT=y` だけが `vendor/intel-vbt/` から include。`intel/provenance/` はユーザー決定で削除）。どのファイルも zedBSD の著作権行、出典の licence block、出典を書いた説明、定義の順。表示の定義は DRM 接頭辞を外した名前（`dp_*`、`FORMAT_*` など）。手で管理 | 旧 `linux/*.inc`、`vk/linux/*.inc`、`parity/firmware_*.c`、`parity/*_ranges.inc` など |
| 試験 | `tests/`（`contracts/`、`execution/`、`display/`、`render/`、`fixtures/`）。試験 build は `I915_TESTS=y`＋`-DI915_TEST_SCENARIO=<name>`（`vmunix.mk`）。QEMU passthrough の試験 build は `I915_TEST_VBT=y` で対象機 VBT（`vendor/intel-vbt/`）を持つ（`XXX:` 実機試験に移ったら削除、`vkloop-hw.sh` は常に付ける）。host 試験は `plan/ws031/tests/`。S5 で移設中 | 旧 `parity/ktest.c`、`eu_test.c`、`parity/tests/`、`dp_fake_hw.c`、`lcd_*ktest.c` ほか |
