...
| P5.edp.1 | intel_dp.c:intel_edp_init_connector → intel_pps.c:intel_pps_init | eDP child | PPS 選択（VBT controller）、delay = max(BIOS レジスタ, VBT)／両方 0 なら eDP 仕様、レジスタ書込み、firmware が残した VDD の引き取り | `parity/dp/intel_pps_port.c`（生成）＋`parity_edp.c` | PORTED／**位置差あり** | RUNTIME（E-108 実機 ×2） | **位置差**: 正本は `intel_setup_outputs()` 内。今は P7 後の試験モード `PARITY_AUX_TEST`。常駐デバイス化で正位置へ |
| P5.edp.2 | intel_dp_aux.c:intel_dp_aux_xfer／transfer、drm_dp_helper.c:drm_dp_dpcd_*／drm_dp_i2c_* | AUX 転送ごと | AUX domain 取得 → PPS lock → VDD on（initializing 中は維持）→ 5 回試行 → status 解釈 → put | `parity/dp/intel_dp_aux_port.c`、`drm_dp_helper_port.c`（生成） | PORTED | RUNTIME | 差: `intel_display_power_put_async`（100 ms 後に worker が put）は即時 put。AUX 完了割り込みは使わずレジスタ polling（正本 6.8 の `intel_dp_aux_wait_done` も polling） |
| P5.edp.3 | intel_pps.c:edp_panel_vdd_work／edp_panel_vdd_schedule_off | VDD 不要になった後 | power_cycle_delay×5 後に worker が VDD off＋AUX 参照返却 | `dp_compat.h` の delayed work（期限の記録）＋`parity_edp_run_due_work()` | **PARTIAL** | FAKE のみ | worker は timer 未接続。停止経路 `intel_pps_vdd_off_sync`（cancel-sync）は PORTED で、試験モードはこれで必ず VDD を落とす。常駐時に worker／timer へ接続 |
| P5.edp.4 | intel_dp.c:intel_edp_init_dpcd の残り、drm_edid の mode 構築、intel_panel／intel_backlight | — | sink rate 表、PSR／DSC／MSO caps、DPCD quirk、fixed mode、backlight setup | なし | UNIMPLEMENTED | — | LCD-A の前提。`intel_hpd_enable_detection`、shared-AUX の HPD 確認も未 |
