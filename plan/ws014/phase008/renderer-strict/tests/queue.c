...
/*
 * Exercises the paired renderer's actual queue dispatch and completion code.
	...
	void)
{
	...
	/* Ordinary and sparse operations must both wait on their actual supplied native fence. */
	...
	fixture_case(1U, VK_SUCCESS, VK_ERROR_OUT_OF_HOST_MEMORY);
	fixture_case(0U, VK_ERROR_DEVICE_LOST, VK_SUCCESS);
	fixture_case(1U, VK_ERROR_OUT_OF_HOST_MEMORY, VK_SUCCESS);

	/* A timeout is still pending and cannot retire until a later successful native observation. */
	fixture_init();
	fixture_dispatch(0U, VK_SUCCESS, 1U);
	fixture_wait_results[0] = VK_TIMEOUT;
	fixture_wait_results[1] = VK_SUCCESS;
	fixture_wait_count = 2U;
	accepted = vkr_queue_sync_submit(&fixture_queue, 1U, 3U, 51U);
	assert(accepted);
	vkr_queue_thread(&fixture_queue);
	assert(fixture_wait_cursor == 2U);
	assert(fixture_callbacks == 1U);
	assert(fixture_last_id == 51U);
	fixture_cleanup();

	/* A later ready ordinary fence cannot overtake an earlier pending sparse submission. */
	fixture_init();
	fixture_ordered = 1U;
	fixture_dispatch(1U, VK_SUCCESS, 1U);
	accepted = vkr_queue_sync_submit(&fixture_queue, 1U, 3U, 91U);
	assert(accepted);
	fixture_fence.base.handle.fence = (VkFence)(uintptr_t)0xf00dU;
	fixture_dispatch(0U, VK_SUCCESS, 1U);
	accepted = vkr_queue_sync_submit(&fixture_queue, 1U, 3U, 92U);
	assert(accepted);
	fixture_wait_results[0] = VK_TIMEOUT;
	fixture_wait_results[1] = VK_SUCCESS;
	fixture_wait_results[2] = VK_SUCCESS;
	fixture_wait_count = 3U;
	vkr_queue_thread(&fixture_queue);
	assert(fixture_wait_cursor == 3U);
	assert(fixture_callbacks == 2U);
	assert(fixture_last_id == 92U);
	assert(fixture_native_submits == 2U);
	fixture_cleanup();

	/* Failure remains sticky even when the next native submission itself reports success. */
	fixture_init();
	fixture_dispatch(0U, VK_SUCCESS, 1U);
	fixture_wait_results[0] = VK_ERROR_DEVICE_LOST;
	accepted = vkr_queue_sync_submit(&fixture_queue, 1U, 3U, 61U);
	assert(accepted);
	vkr_queue_thread(&fixture_queue);
	fixture_dispatch(0U, VK_SUCCESS, 1U);
	accepted = vkr_queue_sync_submit(&fixture_queue, 1U, 3U, 62U);
	assert(!accepted);
	assert(fixture_callbacks == 0U);
	fixture_cleanup();

	/* A marker with no actual submission fence may not fabricate an empty queue success. */
	fixture_init();
	fixture_dispatch(1U, VK_SUCCESS, 0U);
	accepted = vkr_queue_sync_submit(&fixture_queue, 1U, 3U, 71U);
	assert(!accepted);
	assert(fixture_callbacks == 0U);
	assert(fixture_native_submits == 1U);
	fixture_cleanup();

	/* Stopping before any native wait leaves every pending watermark unacknowledged. */
	fixture_init();
	fixture_dispatch(0U, VK_SUCCESS, 1U);
	accepted = vkr_queue_sync_submit(&fixture_queue, 1U, 3U, 81U);
	assert(accepted);
	fixture_queue.sync_thread.join = true;
	error = thrd_create(&fixture_queue.sync_thread.thread, vkr_queue_thread, &fixture_queue);
	assert(error == thrd_success);
	vkr_queue_sync_thread_fini(&fixture_queue);
	assert(fixture_wait_cursor == 0U);
	assert(fixture_callbacks == 0U);
	assert(atomic_load(&fixture_context.queue_sync_failed));

	/* The production fini consumed these synchronization primitives before test cleanup. */
	...
	/* Succeeded: no error, absent fence or unchecked teardown became a successful retirement. */
	puts("STRICT QUEUE actual-fence submit/sparse/ordered-prefix/timeout/error/reuse/teardown PASS");
	return 0;
}
	...
	void)
{
	...
	atomic_init(&fixture_context.queue_sync_failed, false);
	...
	void)
{
	...
	while (true) {
		...
		assert(sync->fence == VK_NULL_HANDLE);
		list_del(&sync->head);
		free(sync);
	}
	...
	VkResult wait_result)
{
	...
	/* Each scenario begins with no previous watermark or native fence. */
	fixture_init();
	...
	/* The marker itself must not issue an extra empty native submission. */
	assert(fixture_native_submits == 1U);
	fixture_cleanup();
}
	...
	VkFence fence)
{
	...
	if (fixture_ordered != 0U)
		...
		assert(fence == VK_NULL_HANDLE || fence == (VkFence)(uintptr_t)0xfaceU);
	...
	uint64_t timeout)
{
	...
	if (fixture_ordered != 0U && fixture_wait_cursor == 2U) {
		...
		assert(fences[0] == (VkFence)(uintptr_t)0xfaceU);
		assert(fixture_callbacks == 0U);
	}
	assert(all == VK_TRUE);
