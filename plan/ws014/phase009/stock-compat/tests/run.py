#!/usr/bin/env python3
"""Run fixed stock source counterexamples locally with existing Mesa build helpers."""
import argparse
import hashlib
import json
...
def main():
    ...
    hashes = {str(path.relative_to(source)): hashlib.sha256(path.read_bytes()).hexdigest()
              for path in (source / "src/venus/vkr_queue.c", source / "src/venus/vkr_queue.h",
                           source / "src/proxy/proxy_context.c")}
    report = {"scope": "fixed stock 1.1.0 source, native API mocks and private socketpair only",
