...
main(void)
{
	...
	assert(error == 0);
	error = fence_bind_call(&receiver, wait_created.fd, 1U, 0U, 0U);
	assert(error == 0);
	...
	handle = handle_fd_get(signal_created.fd, KERNEL_HANDLE_FENCE);
	assert(handle != NULL);
	...
	unsigned flags)
{
	...
	/* The same integer now names a different valid kernel fence, proving a late lookup would be wrong. */
	...
	error = fence_state_call(reuse_receiver, GPU_FENCE_SIGNAL, reuse_wait_fd, 1U, 0, &state);
	assert(error == 0);
