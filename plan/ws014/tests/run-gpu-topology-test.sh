...
# Exercise real GPU/fd/fence cores with independent display event observers.
set -eu
repo=$(CDPATH= cd -- "$(dirname -- "$0")/../../.." && pwd)
work=$(mktemp -d /tmp/zedbsd-gpu-topology.XXXXXX)
...
for mode in ordinary sanitize; do
    ...
    cc -D__UINT64_C_SUFFIX__=UL -std=c11 -O1 -g -Wall -Wextra -Werror -ffunction-sections -fdata-sections $extra \
        -DKERN_USER_ABI_LP64 -I"$repo/include" -I"$repo/src" -I"$repo/libc/include" \
        "$repo/plan/ws014/tests/gpu-topology.c" "$repo/plan/ws014/tests/gpu-test-fd.c" \
        "$repo/src/drivers/gpu/gpu.c" "$repo/src/kern/cdev.c" \
        "$repo/src/kern/fence.c" "$repo/src/kern/handle.c" \
