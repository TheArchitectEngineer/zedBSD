...
main(void)
{
	...
	fail_allocation = 1U;
	error = kernel_fence_create(73U, 0U, &handle);
	assert(error == ENOMEM && handle == NULL);
	error = kernel_fence_create(73U, 0U, &handle);
	assert(error == 0);
	error = kernel_fence_device(handle, 74U);
	assert(error == EXDEV);
	error = kernel_fence_query(handle, 0U, &state);
	assert(error == 0 && state.generation == 1U && state.state == KERNEL_FENCE_PENDING);

	/* A bound producer owns a strong reference independent from every process descriptor. */
	error = kernel_fence_bind(handle, 1U, &producer_owner);
	assert(error == 0);
	error = kernel_fence_signal(handle, 1U, &foreign_owner, 0);
	assert(error == EPERM);
	error = kernel_fence_reset(handle, 1U, &state);
	assert(error == EBUSY);
	error = kernel_fence_wait(handle, 1U, 0U, 1U, &state);
	assert(error == EAGAIN);
	error = kernel_fence_wait(handle, 1U, 10U, 0U, &state);
	...
	received = handle_fd_get(imported, KERNEL_HANDLE_FENCE);
	...
	error = kernel_fence_signal(handle, 1U, &producer_owner, 0);
	assert(error == 0);
	error = kern_poll_wait(&receiver, &descriptor_poll, 1U, 0U, 1, &ready);
	assert(error == 0 && ready == 1 && descriptor_poll.revents == POLLIN);
	error = kernel_fence_wait(received, 1U, 0U, 1U, &state);
	assert(error == 0 && state.state == KERNEL_FENCE_SIGNALED);

	/* Reset mutates the shared reference payload; it never replaces just one alias's wrapper. */
	error = kernel_fence_reset(received, 1U, &state);
	...
	error = kernel_fence_bind(handle, generation, &producer_owner);
	assert(error == 0);
	error = kernel_fence_unbind(handle, generation, &producer_owner);
	assert(error == 0);
	error = kernel_fence_query(received, generation, &state);
	assert(error == 0 && state.state == KERNEL_FENCE_PENDING);

	/* A producer's terminal failure remains distinguishable from successful execution. */
	error = kernel_fence_bind(handle, generation, &producer_owner);
	assert(error == 0);
	error = kernel_fence_signal(handle, generation, &producer_owner, ENODEV);
	assert(error == 0);
	handle_put(handle);
	error = kern_poll_wait(&receiver, &descriptor_poll, 1U, 0U, 1, &ready);
	assert(error == 0 && ready == 1 && descriptor_poll.revents == (POLLIN | POLLERR));
	error = kernel_fence_wait(received, generation, 0U, 1U, &state);
	assert(error == 0 && state.state == KERNEL_FENCE_ERROR && state.error == ENODEV);
	...
	puts("kernel fence: SCM_RIGHTS, exact-generation reset, producer authority, timeout and level poll PASS");
