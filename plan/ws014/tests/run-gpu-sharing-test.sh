#!/bin/sh
# Exercise real GPU, handle and descriptor cores with bounded allocation peers.
set -eu
repo=$(CDPATH= cd -- "$(dirname -- "$0")/../../.." && pwd)
work=$(mktemp -d /tmp/zedbsd-gpu-sharing.XXXXXX)
...
for mode in ordinary sanitize; do
    ...
    cc -std=c11 -O1 -g -Wall -Wextra -Werror -ffunction-sections -fdata-sections $extra \
        ...
        "$repo/plan/ws014/tests/gpu-sharing.c" "$repo/plan/ws014/tests/gpu-test-fd.c" \
