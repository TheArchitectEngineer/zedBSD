#!/bin/sh
# Exercise authoritative GPU jobs with actual GPU/fd/fence ownership.
set -eu
repo=$(CDPATH= cd -- "$(dirname -- "$0")/../../.." && pwd)
work=$(mktemp -d /tmp/zedbsd-gpu-job.XXXXXX)
...
for mode in ordinary sanitize; do
    ...
    cc -std=c11 -O1 -g -Wall -Wextra -Werror -Wno-unused-function -ffunction-sections -fdata-sections $extra \
        ...
        "$repo/plan/ws014/tests/gpu-job.c" "$repo/plan/ws014/tests/gpu-test-fd.c" \
