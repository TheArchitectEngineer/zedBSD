# ws032-p004 結果: libc・ヘッダの不足補完（2026-09-21、2026-09-23 cleared）

q315-i04。**p004 は一度で閉じない**。不足は後続 Phase の実ビルドが見つけるたびにここへ
戻して閉じる。この差し戻しは WS032 の終了（2026-09-23）で終わり、p004 は cleared とする。
下の §1 は p006（OpenSSL）が要求した分で、p005・p007・p008 が後から要求した分は
[user-requested-fixes.md](../user-requested-fixes.md) と [p008 の結果](../phase008/results.md)
にある。以下は p006（OpenSSL）のビルドが実際に要求したものだけで、推測で足した
ものは無い。
