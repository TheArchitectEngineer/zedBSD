#!/bin/sh
# Exercise real command batching and real context framing through independent peers.
set -eu
root=$(CDPATH= cd -- "$(dirname -- "$0")/../../.." && pwd)
work=$(mktemp -d /tmp/zedbsd-vulkan-command-context.XXXXXX)
...
for mode in normal sanitized; do
    ...
    "$cc" -std=c89 -D_POSIX_C_SOURCE=200809L -Wall -Wextra -Werror -pedantic \
        ...
        "$root/plan/ws030/tests/libvulkan-command-context.c" -o "$work/context-$mode"
