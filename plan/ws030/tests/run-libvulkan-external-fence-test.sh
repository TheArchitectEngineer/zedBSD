...
cc -std=c89 -Wall -Wextra -Werror -D_POSIX_C_SOURCE=200809L -Dioctl=fence_test_ioctl -Dpoll=fence_test_poll -Dfcntl=fence_test_fcntl -Dclose=fence_test_close \
    ...
    "$repo/plan/ws030/tests/libvulkan-external-fence.c" \
...
cc -std=c89 -Wall -Wextra -Werror -D_POSIX_C_SOURCE=200809L -Dioctl=fence_test_ioctl -Dpoll=fence_test_poll -Dfcntl=fence_test_fcntl -Dclose=fence_test_close \
    ...
    "$repo/userland/base/libvulkan/query.c" \
    "$repo/plan/ws030/tests/libvulkan-external-fence.c" \
    -pthread -o "$work/external-fence-sanitized"
ASAN_OPTIONS=detect_leaks=1 timeout 20 "$work/external-fence-sanitized"
