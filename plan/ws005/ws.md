...
Status: incomplete。今回追加されたネットワーク改善1〜3を担当する。既存p001〜p012の完了・受け入れは維持する。新しいQueue・実装はない。
Primary Milestone: MG005。Related Milestone: MG006（DE状態通知・表示反映）。Focused Goal: fg005。旧Priorityリストは削除したまま。

1. net lan enable/disableとnetworkdによる有線管理。後挿しでIP設定/DHCPを開始。
2. oneshot network-enableでLAN/Wi-Fiの両方をenable。有線またはWi-FiのどちらかでIP取得したら終了。設定可能な最大待機、既定30秒。timeoutでも起動とnetworkdの接続処理を継続。
3. networkdの状態変化をunix socketまたはファイルでDEへ提供し、受信側で状態を反映。

net wifi enableの現行sourceはポリシー有効化とバックグラウンド接続開始を行う。IP取得待機とは分離する。詳細・確定回答・提案・未決入力は `plan/ws005/network-improvements-2026-09-11.md` に保存（ローカル、公開待ち）。p013で共通契約、p014でLANと状態query、p015で起動待機、p016で通知/DE、p017で統合と最終規約確認。
新規開発のためWSの現在状態をincompleteとするが、過去の完了を取り消さない。WS011/WS012の既存契約を利用し、閉鎖済みWS002/WS019/WS025は再開しない。
...
Historical acceptance: p001--p011 complete (`q071`); p012 complete (`q085`). Current WS status is incomplete for the new p013–p017 plan.
...
Current result: [p012](phase012/phase.md) under q085 is
complete after the acceptance release. See [30 individual results](phase012/results.md)
and [review dispositions](phase012/review2-response.md).
...
| --- | --- | --- |
| [`ws005-p001`](phase001/phase.md) | Complete (`q029`) | RTL8156 NCM carrier/static/DHCP/ping and final Latitude external fetch pass |
| [`ws005-p002`](phase002/phase.md) | Complete (`q053`) | Frozen v1 topology, ownership, security, supersession, limits, and recovery semantics are synchronized across the dependent P-books; no source or hardware result is claimed |
| [`ws005-p003`](phase003/phase.md) | Complete (`q040`) | Fixed 12-byte connection-time AF_UNIX identity, checked `root:network 0660` publication, root/non-root operation policy, and kernel ioctl privilege boundary pass focused and native PC-98 gates |
| [`ws005-p004`](phase004/phase.md) | Complete (`q059`) | Six direct-root `/sbin/wifi` forms pass focused bounds/secret-clearing gates and the physical normal path without taking DHCP or persistence ownership |
| [`ws005-p005`](phase005/phase.md) | Complete (`q051`) | Real root/sudo-like/non-root `/sbin/net wifi set-key`, read-side replacement rejection, metadata, redaction, atomic update, abrupt stop, and second-boot persistence pass |
| [`ws005-p006`](phase006/phase.md) | Complete (`q071`) | Bounded ZNV2 global WLAN requests, authenticated policy-owner selection, daemon-side fixed-store reads, and the secret-FD child path pass |
| [`ws005-p007`](phase007/phase.md) | Complete (`q071`) | The six global `net wifi` forms, deterministic all-WLAN selection, and one managed connection pass |
| [`ws005-p008`](phase008/phase.md) | Complete (`q071`; user accepted 2026-09-05) | The consolidated real-hardware WLAN result passes; explicit WS acceptance supersedes the older five-run campaign |
| [`ws005-p009`](phase009/phase.md) | Complete (`q059`) | One USB-passthrough development run reached scan, authorized carrier, DHCP, two 3/3 ping checks, an 84255-byte fetch, disconnect, and down |
